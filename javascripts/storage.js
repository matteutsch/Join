...
async function setItem(key, value) {
  // ("contacts", contacts) or ("tasks", tasks)  or ("activeContact", activeContact)
...
a = async function await getItem('tasks')
...
=======
async function getItemTasks(key) {
  // ("contacts",contacts) or ("tasks", tasks)  or ("activeContact", aktiveContact)
  const url = `${STORAGE_URL}?key=${key}&token=${STORAGE_TOKEN}`;
  let res2 = await fetch(url).then((res) => res.json()); // response in converted in JSON
  let dataString = await res2.data.value;
  let data = await dataString.replace(/'/g, '"');
  /* return await JSON.parse(data); */
  tasks = await JSON.parse(data);
>>>>>>> 17642a28e4e414fca544759fee14ef9ddc44a552
}
...
=======
async function getItemActiveContact(key) {
  // ("contacts") or ("tasks")  or ("activeContact")
  const url = `${STORAGE_URL}?key=${key}&token=${STORAGE_TOKEN}`;
  let res2 = await fetch(url).then((res) => res.json()); // response in converted in JSON
  let dataString = await res2.data.value;
  let data = await dataString.replace(/'/g, '"');
